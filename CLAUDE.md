...
| `DEFAULT_OWNER_USER_ID` | — (required) | Authentik user UUID that Slack/scheduler rows are attributed to, since those code paths have no logged-in session (see `constants.py`) |
