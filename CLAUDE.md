...
All four are read once and cached at module level (same pattern as `db._engine` being built once at import). Requires a Slack app already created at api.slack.com with **Socket Mode** enabled (an App-Level Token with `connections:write`), a bot token with `chat:write` scope, and Interactivity turned on — this repo does not create or configure the Slack app itself. Requires an OAuth2 Provider + Application already created in Authentik (redirect URI `<app base URL>/auth/callback`) — this repo does not create it itself; see `datacarebot-identity/authentik/scripts/create_oauth2_client.py`.
...
**Currently disabled** — `start_bot()`/`start_scheduler()` log a warning and return immediately without doing anything. There is no link between a Slack identity and an Authentik user (`owner_user_id` has no default anymore — see `TODO(chat-identity)` at the top of `slack_bot.py`), so nothing here can safely attribute an action to an owner. The flow below describes how it worked before being disabled and how it's meant to work again once chat identity linking is designed; the code is intact for that, just not reachable.

1. `reminder_scheduler.py:start_scheduler()` would be started from `main.py` at app startup (background `APScheduler` job, interval `REMINDER_CHECK_INTERVAL_MINUTES`, default 10 min) — guarded against double-start under Flask's debug reloader via the `WERKZEUG_RUN_MAIN` env var.
...
3. The reminder DM has a "Log now" button (`slack_bot.py`, Slack Bolt, Socket Mode). Clicking it opens a modal pre-filled via `db.py:get_next_default_preset()` — the habitual `meal_default_item` preset for that `meal_type`/day, rotated to the next configured preset if that meal type's already been logged N times this week (`N % number_of_presets`). Each food-item row is a searchable `static_select` (Slack's built-in type-to-filter) sourced from `food_characteristics`, ordered by `db.py:get_food_types_ranked_by_usage()` — food_types logged for that specific `meal_type` in the last 14 days first (most-used first), then the rest of the catalog alphabetically, labeled by the English `food_type` (not `food_type_es`) — plus a trailing "Other (new food)" option that reveals a free-text block for a food not in the catalog yet. The modal also supports adding more rows ("+ Add food item"), removing a pre-filled or added row ("Remove food item" — hidden while only one row remains, since submit still requires at least one resolved food_type), and a native Submit button.
...
5. The same modal flow can also be triggered on demand, without waiting for a missed-meal reminder, via the `/register-food` Slack slash command (`slack_bot.py:handle_register_food_command()`): it always opens a small meal-type picker modal first (`register_food_meal_type_modal` — a `static_select` over the four `meal_type` values, ignoring any text typed after the command so there's no ambiguity around accepted values), then on submit pushes (`response_action="push"`, no fresh `trigger_id` needed) the exact same food-item modal `open_meal_form` builds, defaulting `meal_date` to "today" in `APP_TIMEZONE` (`slack_bot.py:_local_today()` — duplicates `reminder_scheduler.py`'s tz resolution locally rather than importing it, since `reminder_scheduler` already imports `slack_bot` and a reverse import would be circular). Requires the `/register-food` slash command to be added under "Slash Commands" in the Slack app's own configuration (api.slack.com) — this repo does not create that command definition itself, only handles it once configured.
6. `/meal_default_presets` — an admin UI (same minimalist per-row-AJAX style as `/meal_schedule`/`/food_characteristics`) for editing/adding/deleting `meal_default_item` rows: which foods+quantities are habitual for a given meal type, day of week, and preset order.
...
| `src/food_recognition/auth.py` | OIDC login (Authlib) against Authentik — `auth` blueprint (`/login`, `/auth/callback`, `/logout`), the `login_required` decorator applied to every route in `main.py`, and `current_user()`. Also validates Bearer access tokens (RS256 JWT, verified locally against Authentik's JWKS) as an alternative to the browser session — same `login_required`/`current_user()`, for a future API client (mobile app/SPA) calling the same routes without a session cookie |
...
- **`meal_schedule`** — the habitual time range (`start_time`–`end_time`) for each `meal_type`, split by `is_weekend`, **per owner** (`owner_user_id`, the Authentik `sub` claim from `session['user']`). `uuid` is the per-row PK (same client-side generation pattern as `food_register`); `meal_type` is a foreign key into `meal_type`; `UNIQUE(meal_type, is_weekend, owner_user_id)` allows at most one range per combination per owner (6 rows per owner). Each owner is lazily seeded with the default ranges the first time their schedule is requested (`db.py:_seed_meal_schedule()`/`get_meal_schedule()`), not by `db.sync_schema()`. Editable from the UI at `/meal_schedule`. Used to auto-classify `food_register.meal_type` at insert time for that same owner (see `db.py:get_meal_type_for_time()`) and would drive the reminder scheduler (currently disabled — see "Slack meal-reminder flow").
- **`meal_default_item`** — habitual food items for a `(meal_type, day_of_week)`, **per owner** (`owner_user_id`), grouped into ordered presets (`preset_order`) with items ordered within a preset by `item_order`; no separate preset "header" row — a preset is just the set of rows sharing `(meal_type, day_of_week, preset_order, owner_user_id)`. Each owner is lazily seeded with one default breakfast preset (milk + banana, all 7 days) as a starting example the first time their presets are requested (`db.py:_seed_meal_default_items()`/`get_meal_default_items()`); lunch/dinner start empty. Editable from `/meal_default_presets`. Used by `db.py:get_next_default_preset()` to pick which of that owner's presets to pre-fill in the Slack modal (rotated weekly — see "Slack meal-reminder flow").
...
| `APP_TIMEZONE` | `UTC` | Timezone the reminder scheduler uses to resolve "today"/weekday (no per-request cookie in a background job) — currently unused, the scheduler is disabled, see `reminder_scheduler.py:start_scheduler()` |
