...
- **`meal_schedule`** — the habitual time range (`start_time`–`end_time`) for each `meal_type`, split by `is_weekend`, **per owner** (`owner_user_id`). `uuid` is the per-row primary key (same client-side generation pattern as `food_register`); `meal_type` is a foreign key into the `meal_type` table; `UNIQUE(meal_type, is_weekend, owner_user_id)` allows at most one range per combination per owner. Each owner is lazily seeded with reasonable default ranges the first time their schedule is requested, editable from the UI at `/meal_schedule`. Used to auto-classify `food_register.meal_type` at insert time for that same owner, and to drive the reminder scheduler (currently always against a single default owner — see `constants.py:DEFAULT_OWNER_USER_ID`).
...
| `DEFAULT_OWNER_USER_ID` | — (required) | Authentik user UUID that Slack/scheduler rows are attributed to, since those code paths have no logged-in session |
