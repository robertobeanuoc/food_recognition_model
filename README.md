...
The schema is kept in sync with these models programmatically: `db.sync_schema()` runs once at app startup (`main.py`) and creates any table that's missing (via `Base.metadata.create_all()`), drops the legacy UUID `BEFORE INSERT` triggers if a database still has them from before uuid generation moved client-side, and seeds the global `meal_type` reference table with default data if it's empty. It never alters or drops existing tables/columns — it's schema creation, not a migration tool. `meal_schedule` and `meal_default_item` are per-owner (`owner_user_id`) and are seeded lazily instead, the first time that owner's rows are requested.

### Slack meal-reminder flow

**Currently disabled** — `start_bot()`/`start_scheduler()` log a warning and return immediately. There's no link between a Slack identity and an Authentik user, so nothing here can safely attribute an action to an owner (see `TODO(chat-identity)` at the top of `slack_bot.py`). The flow below is how it worked before being disabled, kept for when chat-identity linking is designed and this gets re-enabled.

1. `reminder_scheduler.py:start_scheduler()` would start a background `APScheduler` job from `main.py` at startup (interval `REMINDER_CHECK_INTERVAL_MINUTES`, default 10 minutes), guarded against Flask's debug reloader starting it twice.
...
DB, OpenAI, Slack, and OIDC login credentials are read from HashiCorp Vault (KV v2, static-token auth) via `src/food_recognition/vault_client.py`, one secret per path, each read once and cached. When `VAULT_ADDR`/`VAULT_TOKEN` aren't set, DB, OpenAI, and OIDC credentials fall back to plain env vars (`DB_*`, `OPENAI_API_KEY`, `OIDC_*`) — Slack has no such fallback, since the reminder bot only makes sense with Vault configured.
...
# VAULT_OIDC_SECRET_PATH (default: food_recognition/oidc)
cp vault/oidc.example.json oidc.json   # edit oidc.json with real values
vault kv put secret/food_recognition/oidc @oidc.json
rm oidc.json
```
...
The schema is created automatically the first time the app connects — `db.sync_schema()` runs at startup and creates any table declared in `db_models.py` that doesn't exist yet, seeding the global `meal_type` reference table. `meal_schedule` and one starting `meal_default_item` preset (breakfast: milk + banana, every day) are per-owner and seeded lazily instead, the first time each owner's schedule/presets are requested. No manual step is required for a fresh database. The `sql_scripts/tables/*.sql` files are kept as human-readable documentation and a manual fallback:
...
- **`meal_schedule`** — the habitual time range (`start_time`–`end_time`) for each `meal_type`, split by `is_weekend`, **per owner** (`owner_user_id`). `uuid` is the per-row primary key (same client-side generation pattern as `food_register`); `meal_type` is a foreign key into the `meal_type` table; `UNIQUE(meal_type, is_weekend, owner_user_id)` allows at most one range per combination per owner. Each owner is lazily seeded with reasonable default ranges the first time their schedule is requested, editable from the UI at `/meal_schedule`. Used to auto-classify `food_register.meal_type` at insert time for that same owner, and would drive the reminder scheduler (currently disabled — see [Slack meal-reminder flow](#slack-meal-reminder-flow)).
- **`meal_default_item`** — habitual food items for a `(meal_type, day_of_week)`, **per owner** (`owner_user_id`), grouped into ordered "presets" (`preset_order`) with items ordered within a preset by `item_order` — a preset is just the set of rows sharing `(meal_type, day_of_week, preset_order, owner_user_id)`, there's no separate header row. Each owner is lazily seeded with one default breakfast preset (milk + banana, all 7 days) the first time their presets are requested; lunch/dinner start empty. Editable from `/meal_default_presets`. Used by `db.py:get_next_default_preset()` to pick which of that owner's presets to pre-fill in the Slack modal — if a meal's already been logged once this week, the next configured preset is proposed instead of repeating the same one.
...
| `VAULT_OIDC_SECRET_PATH` | `food_recognition/oidc` | Vault KV v2 path for OIDC login credentials |
...
| `APP_TIMEZONE` | `UTC` | Timezone the reminder scheduler uses to resolve "today"/weekday (no per-request cookie in a background job) — currently unused, the scheduler is disabled, see [Slack meal-reminder flow](#slack-meal-reminder-flow) |
